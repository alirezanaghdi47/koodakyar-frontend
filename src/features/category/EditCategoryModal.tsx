...
//===== services =====//
import {editCategory, editCategoryThumbnail} from "@/services/categoryService";
...
export const EditCategoryModalFirstPart = () => {

    const {data: session} = useSession();
    const {modal , isOpenModal , _onOpenModal ,_onCloseModal} = useContext(PortalContext);

    const {mutate, isLoading} = useMutation(editCategoryThumbnail, {
        onSuccess: data => {
        ...
        onError: error => {
            toast.error("خطا در ویرایش آیکن دسته بندی");
        }
    });
...
export const EditCategoryModalSecondPart = () => {

    const queryClient = useQueryClient();
    const {data: session} = useSession();
    const {modal , isOpenModal , _onOpenModal ,_onCloseModal} = useContext(PortalContext);

    const {mutate, isLoading} = useMutation(editCategory, {
        onSuccess: data => {
            if (data.status === "success") {
                toast.success(data.message);
                _onCloseModal();
...
const EditCategoryModal = () => {
    ...
    if (modal.isOpen === true && modal.type === "edit-1")  return <EditCategoryModalFirstPart/>;
    if (modal.isOpen === true && modal.type === "edit-2")  return <EditCategoryModalSecondPart/>;
