...
import {Box, Button, Typography, Collapse, Grid, Stack} from "@mui/material";
import {
    FiAirplay,
    FiBook,
    FiCamera,
...
const AccountSidebar = () => {
    ...
    const _onToggleAccordion = (newExpand) => setExpand(expand === newExpand ? 0 : newExpand);

    const _handleActiveLink = () => {
        const collapsedLinks = navLinks.map(item => item.hasOwnProperty("children") ? ({id: item.id, children: item?.children}) : null).filter(item => item !== null);
